            ...
            if (currentDisplayMode === 'machine') {
                projects.forEach(pNum => {
                    ...
                    machines.forEach(mName => {
                        ...
                        if (tasksInMachine.length === 0) return;
                        ...
                        // unit=ALL以外のユニット名を収集（is_detailed除外）
                        const unitsSet = new Set();
                        tasksInMachine.forEach(t => {
                            const det = t.is_detailed;
                            if (det === true || String(det).toLowerCase() === 'true' || det === 't' || det === '1') return;
                            const u = String(t.unit || "").trim();
                            if (u && u.toUpperCase() !== 'ALL') unitsSet.add(u);
                        });
                        ...
                        if (units.length > 0) {
                            // ユニット固有タスクが存在する場合：unit別サブグループを作成
                            // unit=ALLのタスクは全ユニットに複製して表示
                            const allUnitTasks = tasksInMachine.filter(t =>
                                String(t.unit || "").trim().toUpperCase() === 'ALL' ||
                                String(t.unit || "").trim() === ''
                            );

                            units.forEach(uName => {
                                const unitSpecificTasks = tasksInMachine.filter(t =>
                                    String(t.unit || "").trim() === uName
                                );
                                const tasksInUnit = [...allUnitTasks, ...unitSpecificTasks];
                                tasksInUnit.sort(machineTaskSort);

                                const unitParentKey = `p_${pNum}_m_${mName}_u_${uName}`;
                                let uMinStart = null, uMaxEnd = null;
                                tasksInUnit.forEach(t => {
                                    const start = parseLocalDate(t.start_date);
                                    const end = gantt.calculateEndDate(start, t.duration);
                                    if (!uMinStart || start < uMinStart) uMinStart = start;
                                    if (!uMaxEnd || end > uMaxEnd) uMaxEnd = end;
                                });

                                if (!parentsMap[unitParentKey]) {
                                    parentsMap[unitParentKey] = {
                                        id: unitParentKey,
                                        text: uName,
                                        project_number: pNum,
                                        machine: mName === "設定なし" ? "" : mName,
                                    ...
                                    tasksWithHierarchy.push(parentsMap[unitParentKey]);
                                }

                                tasksInUnit.forEach(t => {
                                    const taskClone = { ...t };
                                    taskClone.id = `${t.id}_${mName}_${uName}`;
                                    taskClone.original_id = t.id;
                                    taskClone.parent = unitParentKey;
                                    tasksWithHierarchy.push(taskClone);
                                });
                            });
                        } else {
                            // ユニットが全てALLまたは空：機械行の直下にタスクを並べる
                            tasksInMachine.sort(machineTaskSort);
                            tasksInMachine.forEach(t => {
                                const taskClone = { ...t };
                                taskClone.id = `${t.id}_${mName}`;
                                taskClone.original_id = t.id;
