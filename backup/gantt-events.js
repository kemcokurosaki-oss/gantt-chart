        ...
        */
        ...
        function _ensureDisplayFilterCache() {
            ...
            gantt.eachTask(function(task) {
                ...
                if (!visible) return;
                // 親チェーンをすべて辿って仮想親行を可視化（ユニット行→機械行の多段階に対応）
                let parentId = task.parent;
                while (parentId != null && parentId !== "" && parentId !== 0) {
                    if (nextVirtualVisible[parentId]) break;
                    nextVirtualVisible[parentId] = true;
                    const parentTask = gantt.isTaskExists(parentId) ? gantt.getTask(parentId) : null;
                    if (!parentTask || !parentTask.$virtual) break;
