...
function Xe(t) {
  ...
  t._isLinksCacheEnabled = function() {
    ...
    return d = l ? c : c.concat(this._getInheritedPredecessors(o)), n && (s[u] = d), d;
  ...
  return { getVertices: function(t) {
    ...
    for (var e = {}, i = [], a = this.groupAdjacentEdges(n), s = !1, r = [], o = 0; o < t.length; o++) {
      ...
      if (!l.visited) for (var d = [l], u = 0; d.length; ) {
        var c = d.pop();
        c.visited || (c.index = u, c.lowLink = u, u++, i.push(c), c.onStack = !0, c.visited = !0), s = !1, n = a[c.id] || [];
        ...
        if (!s) {
          if (c.index == c.lowLink) {
            for (var g = { tasks: [], links: [], linkKeys: [] }; (_ = i.pop()).onStack = !1, g.tasks.push(_.id), _.edge && (g.links.push(_.edge.id), g.linkKeys.push(_.edge.hashSum)), _ != c; ) ;
            r.push(g);
          }
...
function en(t) {
  return { getVirtualRoot: function() {
    ...
    for (var s = [], r = {}, o = {}, l = 0; l < i.length; l++) this._getLinkedTasks(i[l], r, e, o);
    ...
    for (var s, r = n === void 0 ? t.config.root_id : n, o = (e = {}, {}), l = [{ from: r, includePredecessors: i, isChild: !1 }]; l.length; ) {
  ...
  constructor(n) {
    this.isAsapTask = (e) => {
      ...
      if (a.project_constraint && !this._gantt.getState().group_mode) {
        let s = V.ASAP;
        a.schedule_from_end && (s = V.ALAP), (i && i.constraint_type) !== s && i || (i = this._getParentConstraint(e));
      }
      return i;
      ...
      if (a && !(a.constraint_type === V.ALAP || a.constraint_type === V.ASAP)) {
        if (St(a.constraint_date)) {
          const s = a.constraint_date, r = Pt.Create(i);
      ...
      let o;
      if (this._gantt.eachTask((l) => r(l), e), i) for (let l = 0; l < i.length; l++) {
        const d = i[l];
...
class je {
  ...
  resolveRelationDate(n, e, i) {
    ...
    for (let _ = 0; _ < d.length; _++) {
      const g = d[_];
      o = g.preferredStart;
      const y = this.getLatestEndDate(g, i, l), m = this._gantt.calculateEndDate({ start_date: y, duration: -l.duration, task: l });
    ...
    const h = Pt.Create(c);
    return h.link = s, h.task = n, h.end_date = a, h.start_date = r, h.kind = "alap", u && (h.latestSchedulingStart = this._gantt.calculateEndDate({ start_date: u, duration: -l.duration, task: l }), h.latestSchedulingEnd = u), h;
  }
  getSuccessorStartDate(n, e) {
    const i = e[n], a = this._gantt.getTask(n);
    let s;
    return s = i && (i.start_date || i.end_date) ? i.start_date ? i.start_date : this._gantt.calculateEndDate({ start_date: i.end_date, duration: -a.duration, task: a }) : a.start_date, s;
  }
  getLatestEndDate(n, e, i) {
    const a = this.getSuccessorStartDate(n.target, e), s = i;
    let r = this._gantt.getClosestWorkTime({ date: a, dir: "past", task: s });
    return r && n.lag && 1 * n.lag == 1 * n.lag && (r = this._gantt.calculateEndDate({ start_date: r, duration: 1 * -n.lag, task: s })), r;
  }
}
class Fe {
  static Create(n) {
  ...
  resolveRelationDate(n, e, i) {
    ...
    const _ = i[n], g = Pt.Create(_);
    return g.link = s, g.task = n, g.start_date = a, g.end_date = h, g.kind = "asap", c && (g.earliestSchedulingStart = c, g.earliestSchedulingEnd = this._gantt.calculateEndDate({ start_date: c, duration: o.duration, task: o })), g;
  }
  getPredecessorEndDate(n, e) {
    const i = e[n], a = this._gantt.getTask(n);
    let s;
    return s = i && (i.start_date || i.end_date) ? i.end_date ? i.end_date : this._gantt.calculateEndDate({ start_date: i.start_date, duration: a.duration, task: a }) : a.end_date, s;
  }
  getEarliestStartDate(n, e, i) {
    const a = this.getPredecessorEndDate(n.source, e), s = i, r = this._gantt.getTask(n.source), o = this._gantt._getAutoSchedulingConfig();
    let l;
    if (a && n.lag && 1 * n.lag == 1 * n.lag) {
      let d = s;
      o.move_projects && n.subtaskLink && this._gantt.isTaskExists(n.targetParent) && (d = this._gantt.getTask(n.targetParent)), l = this._gantt.getClosestWorkTime({ date: a, dir: "future", task: r }), n.sourceLag && (l = this._gantt.calculateEndDate({ start_date: l, duration: 1 * n.sourceLag, task: r })), n.targetLag && (l = this._gantt.calculateEndDate({ start_date: l, duration: 1 * n.targetLag, task: d })), l = this._gantt.calculateEndDate({ start_date: l, duration: 1 * n.trueLag, task: d });
    } else {
...
class di {
  ...
  generatePlan(n, e) {
    const i = this._graphHelper, a = this._gantt, s = this._constraintsHelper, r = this._alapStrategy, o = this._asapStrategy, l = a._getAutoSchedulingConfig(), { orderedIds: d, reversedIds: u, relationsMap: c, plansHash: h } = this.buildWorkCollections(n, e, i);
    let _;
  ...
  applyProjectPlan(n) {
    ...
    for (let l = 0; l < n.length; l++) {
      if (s = null, r = null, i = n[l], !e.isTaskExists(i.task)) continue;
      a = e.getTask(i.task), i.link && (s = e.getLink(i.link), r = i.kind === "asap" ? this._gantt.getTask(s.source) : this._gantt.getTask(s.target));
      let d = null;
      i.start_date && a.start_date.valueOf() !== i.start_date.valueOf() && (d = i.start_date), d && (a.start_date = d, a.end_date = e.calculateEndDate(a), o.push(a.id), e.callEvent("onAfterTaskAutoSchedule", [a, d, s, r]));
    }
    return o;
  }
  iterateTasks(n, e, i, a, s, r, o) {
    ...
    if (this._secondIterationRequired) {
      ...
      else if (this._secondIteration = !0, this.summaryLagChanged(l, r, o)) return this.iterateTasks(n, e, i, a, s, r, o);
    }
    return d;
  }
  summaryLagChanged(n, e, i) {
    ...
    for (const o in e) e[o].predecessors.forEach((l) => {
      if (l.subtaskLink) {
        ...
        this.getProjectUpdates(n, i, l, d, "source", a, s), this.getProjectUpdates(n, i, l, d, "target", a, s);
      }
    });
    let r = !1;
    for (const o in a) {
  ...
  getProjectUpdates(n, e, i, a, s, r, o) {
    if (n.getTask(a[s]).type === n.config.types.project) {
      ...
      const l = r[a[s]];
      let d = e[i[s]];
      d && (s != "source" || d.start_date && d.end_date || (d = n.getTask(d.task)), l.min = l.min || d.start_date, l.min > d.start_date && (l.min = d.start_date), l.max = l.max || d.end_date, l.max < d.end_date && (l.max = d.end_date), o[a.id] = a);
    }
  }
  processResolvedDate(n, e, i, a) {
    if (e.start_date && this._gantt.isLinkExists(e.link)) {
      let s = null, r = null;
      if (e.link && (s = this._gantt.getLink(e.link), r = e.kind === "asap" ? this._gantt.getTask(s.source) : this._gantt.getTask(s.target)), n.start_date.valueOf() !== e.start_date.valueOf() && this._gantt.callEvent("onBeforeTaskAutoSchedule", [n, e.start_date, s, r]) === !1) return;
    }
  ...
  buildWorkCollections(n, e, i) {
    const a = this._gantt, s = i.topologicalSort(n), r = s.slice().reverse(), o = {}, l = {};
  ...
  processConstraints(n, e, i, a) {
    for (let s = 0; s < n.length; s++) {
      const r = n[s], o = i.getTask(r), l = a.getConstraintType(o);
...
function _e(t, n, e) {
  ...
  for (; i.length > 0; ) if (o = i.shift(), !e[o]) {
    ...
    for (let u = 0; u < n.length; u++) {
      const c = n[u];
      c.source == o || c.sourceParent == o ? (e[c.target] || (i.push(c.target), r[c.id] = !0, n.splice(u, 1), u--), s[c.hashSum] = c) : c.target != o && c.targetParent != o || (e[c.source] || (i.push(c.source), r[c.id] = !0, n.splice(u, 1), u--), s[c.hashSum] = c);
    }
  }
  const l = [];
  let d = [];
...
function Y(t) {
  ...
  if (t.getBoundingClientRect) {
    ...
    n = s.top + l - u, e = s.left + d - c, i = document.body.offsetWidth - s.right, a = document.body.offsetHeight - s.bottom;
  } else {
...
function it(t) {
  if (!t) return "";
  var n = t.className || "";
  return n.baseVal && (n = n.baseVal), n.indexOf || (n = ""), Ae(n);
}
function $t(t, n) {
...
function kt(t, n, e) {
  ...
  for (e === void 0 && (e = !0); i; ) {
    if (a = it(i)) {
      ...
      if (s >= 0) {
        if (!e) return i;
        var r = s === 0 || !Ae(a.charAt(s - 1)), o = s + n.length >= a.length || !Ae(a.charAt(s + n.length));
        if (r && o) return i;
      }
    }
    i = i.parentNode;
...
const Pn = Object.freeze(Object.defineProperty({ __proto__: null, addClassName: $t, closest: ct, getActiveElement: De, getChildNodes: In, getClassName: it, getClosestSizedElement: Ln, getFocusableNodes: Vt, getNodePosition: Y, getRelativeEventPosition: dt, getRelativeNodePosition: Mn, getRootNode: Et, getScrollSize: En, getTargetNode: At, hasClass: function(t, n) {
  return "classList" in t ? t.classList.contains(n) : new RegExp("\\b" + n + "\\b").test(t.className);
}, hasShadowParent: function(t) {
  return !!Et(t);
}, insertNode: An, isChildOf: Z, isShadowDomSupported: Nn, locateAttribute: et, locateClassName: kt, removeClassName: Nt, removeNode: Dn, toNode: We }, Symbol.toStringTag, { value: "Module" })), tt = typeof window < "u" ? window : global;
let mi = class {
  constructor(t) {
  ...
  attach(t, n, e) {
    ...
    this._domEvents.attach(l, "mouseup", (g) => {
      ...
      if (this._touchActive) {
        let y = u();
        if (y && i.utils.dom.closest(g.target, y)) return;
    ...
    const u = () => {
      ...
      r = null;
      let y = u();
      y && i.utils.dom.closest(g.target, y) || (s.registerProvider("clickDrag", () => ({ autoscroll: this._mouseDown })), n && g[n] !== !0 || (r = this._getCoordinates(g, t)));
    }, h = (g) => {
      if (r = null, (!n || g[n] === !0) && this._mouseDown === !0) {
        this._mouseDown = !1;
...
function zn(t) {
  ...
  return t.$services, { getSum: function(e, i, a) {
    ...
    for (var d = this.splitSize(s, e.length), u = a, c = [], h = e.length - 1; h >= 0; h--) {
      ...
      _ && this.processIgnores(g), _ && g.column_width && (u = g.column_width * (g.display_count || g.count)), this.initColSizes(g, i, u, d[h]), this.limitVisibleRange(g), _ && (u = g.full_width), c.unshift(g);
    }
    ...
    var s = H({ count: 0, col_width: 0, full_width: 0, height: 0, width: [], left: [], trace_x: [], trace_indexes: {}, min_date: new Date(i), max_date: new Date(a) }, e);
...
function Ve(t) {
  ...
  return n.processIgnores = function(e) {
    ...
    if (e.ignore_x = {}, t.ignore_time || t.config.skip_off_time) {
      ...
      i = 0;
      for (var s = 0; s < e.trace_x.length; s++) a.call(t, e.trace_x[s]) || this._ignore_time_config.call(t, e.trace_x[s], e) ? (e.ignore_x[e.trace_x[s].valueOf()] = !0, e.ignored_colls = !0) : i++;
    }
    e.display_count = i;
  }, n;
}
...
class bi {
  constructor(n) {
    this.show = (e, i) => {
      ...
      u.innerHTML = h, i.eventRemove(s, "click", this._qiButtonClickHandler), i.eventRemove(s, "keypress", this._qiKeyPressHandler), i.event(s, "click", this._qiButtonClickHandler), i.event(s, "keypress", this._qiKeyPressHandler);
  ...
  _get_event_counter_part(n, e, i, a) {
    ...
    return d ? { left: o, top: l, dx: o + r.offsetWidth / 2 - u.x > i.offsetWidth / 2 ? 1 : 0, dy: l + r.offsetHeight / 2 - u.y > a.offsetHeight / 2 ? 1 : 0, width: r.offsetWidth, height: r.offsetHeight } : null;
  }
  ...
  _show_quick_info(n, e) {
    ...
    if (i.config.quick_info_detached) {
      ...
      const r = a.offsetWidth, o = a.offsetHeight, l = i.getScrollState(), d = s.xViewport, u = s.yViewport, c = d.offsetWidth + l.x - r, h = n.top - l.y + o;
      let _ = n.top;
      h > u.offsetHeight / 2 && (_ = n.top - (o + n.height + 2 * e), _ < l.y && h <= u.offsetHeight && (_ = n.top)), _ < l.y && (_ = l.y);
      const g = Math.min(Math.max(l.x, n.left - n.dx * (r - n.width)), c), y = _;
...
class xi {
  ...
  show(n, e) {
    ...
    if (Z(s, a) || (this.hide(), s.style.top = s.style.top || "0px", s.style.left = s.style.left || "0px", a.appendChild(s)), this._isLikeMouseEvent(n)) {
  ...
  _calculateTooltipPosition(n) {
    const e = this._gantt, i = this._getViewPortSize(), a = this.getNode(), s = { top: 0, left: 0, width: a.offsetWidth, height: a.offsetHeight, bottom: 0, right: 0 }, r = e.config.tooltip_offset_x, o = e.config.tooltip_offset_y, l = document.body, d = dt(n, l), u = Y(l);
    ...
    return s.top < i.top - c ? (s.top = i.top, s.bottom = s.top + s.height) : s.bottom > i.bottom && (s.bottom = i.bottom, s.top = s.bottom - s.height), s.left < i.left ? (s.left = i.left, s.right = i.left + s.width) : s.right > i.right && (s.right = i.right, s.left = s.right - s.width), d.x >= s.left && d.x <= s.right && (s.left = d.x - s.width - r, s.right = s.left + s.width), d.y >= s.top && d.y <= s.bottom && (s.top = d.y - s.height - o, s.bottom = s.top + s.height), s.left < 0 && (s.left = 0), s.right < 0 && (s.right = 0), s;
  }
  _getViewPortSize() {
...
class $i {
  constructor(n) {
    this._listeners = {}, this.tooltip = new xi(n), this._gantt = n, this._domEvents = ue(), this._initDelayedFunctions();
  }
  destructor() {
  ...
  attach(n) {
    ...
    let a = null;
    const s = (r) => {
      const o = At(r), l = ct(o, n.selector);
...
class wi {
  ...
  _attachEvents() {
    ...
    s.attachEvent("onBeforeItemMove", (l, d, u) => (this.isMoveEventsIgnored() || i(), !0)), s.attachEvent("onAfterItemMove", (l, d, u) => (this.isMoveEventsIgnored() || this.onTaskMoved(a(l)), !0)), e.attachEvent("onRowDragStart", (l, d, u) => (this.toggleIgnoreMoveEvents(!0), i(), !0));
    let r = null, o = !1;
...
const Ti = { auto_scheduling: function(t) {
  ...
  Xe(t);
  var e = en(t), i = tn(), a = ze.Create(t), s = new di(t, i, a), r = new ci(t, e), o = new ui(t, i, e);
  ...
  function e() {
    ...
    const s = t.$ui.getView("timeline"), r = new vi(i, t, s);
  ...
  Xe(t);
  var n = function(i) {
    var a = en(i), s = tn(), r = { _freeSlack: {}, _totalSlack: {}, _slackNeedCalculate: !0, _linkedTasksById: {}, _successorsByTaskId: {}, _projectEnd: null, _calculateSlacks: function() {
      ...
      const _ = { _cache: {}, getDist: function(g, y) {
        ...
        {
          const b = i.calculateDuration({ start_date: g.end_date, end_date: y.start_date, task: g });
      ...
      for (const g in h) {
        if (_[g]) continue;
        const y = h[g];
        this._isCompletedTask(y) || y.unscheduled ? c[y.id] = 0 : c[y.id] = i.calculateDuration({ start_date: y.end_date, end_date: this._projectEnd, task: y });
      }
      ...
      for (var c = {}, h = i.getTaskByTime(), _ = 0; _ < l.length; _++) {
        ...
        if (this._isCompletedTask(y)) c[y.id] = 0;
          ...
          for (var g = 0; g < m.length; g++) {
            const v = m[g], f = i.getTask(v.target);
            let p = 0;
  ...
  var e = function(i) {
    return i._isProjectEnd = function(a) {
      ...
      var s = i.bind(function() {
        return this.reset(), !0;
      }, this), r = i.bind(function(l, d) {
        return this._cache && (this._cache[d] = this._cache[l], delete this._cache[l]), !0;
      }, this);
      i.attachEvent("onAfterLinkAdd", s), i.attachEvent("onAfterLinkUpdate", s), i.attachEvent("onAfterLinkDelete", s), i.attachEvent("onAfterTaskAdd", s), i.attachEvent("onTaskIdChange", r), i.attachEvent("onAfterTaskUpdate", s), i.attachEvent("onAfterTaskDelete", s), i.attachEvent("onParse", s), i.attachEvent("onClear", s), i.$data.tasksStore.attachEvent("onClearAll", s), i.$data.linksStore.attachEvent("onClearAll", s);
      var o = function() {
  ...
  t.config.highlight_critical_path = !1, e.init(n), t.isCriticalTask = function(i) {
    ...
    for (var s = 0, r = [], o = {}, l = 0; l < i.$source.length; l++) o[i.$source[l]] = !0;
    for (l = 0; l < a.$target.length; l++) o[a.$target[l]] && r.push(a.$target[l]);
    if (r[0]) for (l = 0; l < r.length; l++) {
      var d = this.getLink(r[l]), u = this._getSlack(i, a, this._convertToFinishToStartLink(d.id, d, i, a, i.parent, a.parent));
      (s > u || l === 0) && (s = u);
    }
    else s = this._getSlack(i, a, {});
    return s;
  ...
  function l() {
    ...
    n() ? o && (u = "onExpand", function() {
      ...
      const c = t.ext.fullscreen.getFullscreenElement(), h = document.body;
      a.modified && (i.padding && (h.style.padding = i.padding), i.paddingTop && (h.style.paddingTop = i.paddingTop), i.paddingRight && (h.style.paddingRight = i.paddingRight), i.paddingBottom && (h.style.paddingBottom = i.paddingBottom), i.paddingLeft && (h.style.paddingLeft = i.paddingLeft), h.style.overflow = i.overflow, i = { overflow: null, padding: null, paddingTop: null, paddingRight: null, paddingBottom: null, paddingLeft: null }, r(a, c.style), a.modified = !1), s.forEach((_) => {
  ...
  (function(n) {
    n.config.keyboard_navigation = !0, n.config.keyboard_navigation_cells = !1, n.$keyboardNavigation = {}, n._compose = function() {
      ...
      e.$keyboardNavigation.getFocusableNodes = Vt, e.$keyboardNavigation.trapFocus = function(i, a) {
        ...
        for (var s = e.$keyboardNavigation.getFocusableNodes(i), r = De(), o = -1, l = 0; l < s.length; l++) if (s[l] == r) {
          o = l;
          break;
        }
        if (a.shiftKey) {
      ...
      e.$keyboardNavigation.KeyNavNode = function() {
        ...
        if (i) {
          ...
          a.callEvent("onBeforeFocus", [i]) !== !1 && i && (i.setAttribute("tabindex", "-1"), i.$eventAttached || (i.$eventAttached = !0, e.event(i, "focus", function(s) {
      ...
      e.$keyboardNavigation.TaskCell = function(i, a) {
        ...
        var a = e.locate(i);
        if (e.isTaskExists(a)) {
          var s = 0, r = et(i, "data-column-index");
      ...
      var i = function(m) {
        ...
        const m = !Z(document.activeElement, n.$container) && document.activeElement.localName != "body";
      ...
      function l(m) {
        if (!n.config.keyboard_navigation || !n.config.keyboard_navigation_cells && r(m)) return !0;
        if (!o(m)) {
          var b, v = e.fromDomElement(m);
          if (v && (e.activeNode instanceof n.$keyboardNavigation.TaskCell && Z(m.target, n.$task) && (v = new n.$keyboardNavigation.TaskCell(v.taskId, e.activeNode.columnIndex)), b = v, n.config.show_grid && n.$ui.getView("grid") && n.config.keyboard_navigation_cells)) {
            const f = m.target.classList.contains("gantt_row"), p = m.target.closest(".gantt_task_line"), k = n.utils.dom.getNodePosition(n.$grid).x, x = k + n.$grid.offsetWidth, $ = n.utils.dom.getNodePosition(document.activeElement).x;
            if (f || p && ($ < k || x < $)) {
      ...
      var d = n.attachEvent("onGanttReady", function() {
        if (n.detachEvent(d), n.$data.tasksStore.attachEvent("onStoreUpdated", function(b) {
          if (n.config.keyboard_navigation && e.isEnabled()) {
            const v = e.getActiveNode(), f = n.$ui.getView("grid");
      ...
      function g(m) {
        for (var b = n.getGridColumns(), v = 0; v < b.length; v++) if (b[v].name == m) return v;
        return 0;
      }
      ...
      var y = {};
      _t(y), n.mixin(y, { addShortcut: function(m, b, v) {
        var f = _(v);
        ...
        if (e.isEnabled()) {
          var m = e.getActiveNode(), b = (f = m) instanceof n.$keyboardNavigation.GanttNode ? "gantt" : f instanceof n.$keyboardNavigation.HeaderCell ? "headerCell" : f instanceof n.$keyboardNavigation.TaskRow ? "taskRow" : f instanceof n.$keyboardNavigation.TaskCell ? "taskCell" : null, v = n.getGridColumns();
          switch (b) {
            case "taskCell":
  ...
  t.ext || (t.ext = {}), t.ext.quickInfo = new bi(t), t.config.quickinfo_buttons = ["icon_edit", "icon_delete"], t.config.quick_info_detached = !0, t.config.show_quick_info = !0, t.templates.quick_info_title = function(a, s, r) {
  ...
  function n(i) {
    ...
    if (i.text) {
      let u = null;
      u = typeof i.text == "function" ? i.text(i) : i.text, u && (t.config.external_render && t.config.external_render.isElement(u) ? (s.innerHTML = "<div class='gantt_marker_content' ></div>", t.config.external_render.renderElement(u, s.querySelector(".gantt_marker_content"))) : s.innerHTML = "<div class='gantt_marker_content' >" + i.text + "</div>");
    }
    return s;
  }
  function e() {
  ...
  t.config.multiselect = !0, t.config.multiselect_one_level = !1, t._multiselect = { _selected: {}, _one_level: !1, _active: !0, _first_selected_when_shift: null, getDefaultSelected: function() {
    ...
    var i = this.getSelected(), a = this.getFirstSelected(), s = !1, r = this.getLastSelected(), o = t.config.multiselect, l = (function() {
      ...
      if (r) {
        if (e) {
          ...
          for (var _ = r; t.getGlobalTaskIndex(_) !== u; ) this.unselect(_, n), _ = u > h ? t.getNext(_) : t.getPrev(_);
          for (_ = e; t.getGlobalTaskIndex(_) !== u; ) this.select(_, n) && !s && (s = !0, a = _), _ = u > c ? t.getNext(_) : t.getPrev(_);
        }
      } else r = e;
    }).bind(this);
    ...
    if (e && a.isActive()) {
      ...
      if (r && r.$level != o.$level && (l = !0), t.config.multiselect_one_level && l && !i.ctrlKey && !i.shiftKey) return !0;
  ...
  return t.ext = t.ext || {}, t.ext.export_api = t.ext.export_api || { _apiUrl: "https://export.dhtmlx.com/gantt", _preparePDFConfigRaw(n, e) {
    ...
    let a = 0;
    for (let s = 0; s < i.length; s++) i[s].name !== "add" && i[s].name !== "buttons" && (n && n.raw && i[s].hide || (e[a] = { id: i[s].template ? "_" + s : i[s].name, header: i[s].label || t.locale.labels["column_" + i[s].name], width: i[s].width ? Math.floor(i[s].width / 4) : "", tree: i[s].tree || !1 }, i[s].name === "duration" && (e[a].type = "number"), i[s].name !== "start_date" && i[s].name !== "end_date" || (e[a].type = "date", n && n.rawDates && (e[a].id = i[s].name)), a++));
    ...
    const o = n.$_source;
    if (!o) return;
    a[n.id] && t.ext.export_api._onCircDependencyFind(r, e, a, s), a[n.id] = !0;
    const l = {};
    for (let d = 0; d < o.length; d++) {
      if (s[o[d]]) continue;
      const u = e[o[d]], c = i[u._target];
      l[c.id] && t.ext.export_api._onCircDependencyFind(u, e, a, s), l[c.id] = !0, t.ext.export_api._clearCircDependencies(c, e, i, a, s, u);
    }
    a[n.id] = !1;
...
function Ai(t) {
  ...
  var i = { init: function() {
    ...
    var r = rn;
    return e() && (r = on), r.date_to_str(a, s, t);
  }, str_to_date: function(a, s) {
    var r = rn;
    return e() && (r = on), r.str_to_date(a, s, t);
...
var Ue = function(t) {
  var n;
  rt.apply(this, [t]), this._branches = {}, this._splitTaskHelper = new Wn(this), this.pull = {}, this.$initItem = function(o) {
    var l = o;
  ...
  return this._attachDataChange(function() {
    ...
    this._indexRangeCache = {}, this._eachItemMainRangeCache = null, e = {}, i = {}, a = {}, s = {}, r = !1, this.eachItem(function(o) {
      var l = this.getParent(o.id);
      o.$open && a[l] !== !1 ? a[o.id] = !0 : a[o.id] = !1, this._isSplitItem(o) && (r = !0, e[o.id] = !0, i[o.id] = !0), r && i[l] && (this._isDefaultItem(o) || this._isInlineChildItem(o)) && (i[o.id] = !0), a[l] || a[l] === void 0 || this._isInlineChildItem(o) ? s[o.id] = !0 : s[o.id] = !1;
    });
  }), this.attachEvent("onFilterItem", function(o, l) {
    var d = !1;
    this._ganttConfig && (d = this._ganttConfig.open_split_tasks);
    var u = s[l.id];
    return r && (u && i[l.id] && !e[l.id] && (u = !!d), i[l.id] && !e[l.id] && (this._isSplitChildItem(l) || (l.$split_subtask = !0))), l.$expanded_branch = !!s[l.id], this._isInlineChildItem(l) && (u = !1), !!u;
...
Ue.prototype = H({ _buildTree: function(t) {
  ...
  1 * i !== i && (i = void 0), rt.prototype._addItemInner.call(this, t, i), this.setParent(t, e), t.hasOwnProperty("$rendered_parent") && this._move_branch(t, t.$rendered_parent), this._add_branch(t, n);
}, _changeIdInner: function(t, n) {
  var e = this.getChildren(t), i = this._searchVisibleOrder[t];
  rt.prototype._changeIdInner.call(this, t, n);
  ...
  for (var e = 0; e < n.length; e++) this._move_branch(n[e], this.getParent(n[e]), null), rt.prototype._removeItemInner.call(this, n[e].id), this._move_branch(n[e], this.getParent(n[e]), null);
}, move: function(t, n, e) {
  ...
  if (!ft(t, e)) {
    ...
    if (n == -1 && (n = o.length + 1), ft(r, e) && this.getBranchIndex(t) == n) return;
    ...
    (u = lt(u, a)) ? o = o.slice(0, n).concat([t]).concat(o.slice(n)) : o.push(t), l.length && (o = o.concat(l)), ft(s.$rendered_parent, r) || ft(r, e) || (s.$rendered_parent = r), this.setParent(s, e), this._branches[e] = o;
  ...
  U(n) || (n = e);
  var i = lt(n, e) || e, a = !1, s = !1, r = null;
  i === e && (this._eachItemMainRangeCache ? (a = !0, r = this._eachItemMainRangeCache) : (s = !0, r = this._eachItemMainRangeCache = [])), a ? this._eachItemCached(t, r) : this._eachItemIterate(t, i, s ? r : null);
...
const Ni = { create: function() {
  var t = H({}, { createDatastore: function(n) {
    var e = (n.type || "").toLowerCase() == "treedatastore" ? Ue : rt;
    if (n) {
      var i = this;
    ...
    if (i && this.isTaskVisible(n)) this.$data.tasksStore.refresh(n, !!this.getState("tasksDnd").drag_id || e === !1), s();
...
function Pi(t) {
  var n = function(u) {
    ...
    if (u.config.scale_offset_minimal) {
      const g = new Ve(u), y = [g.primaryScale()].concat(g.getAdditionalScales());
...
class Ri {
  ...
  clientSideDelete(n, e, i) {
    ...
    let s = !1;
    i.getUserData(n, "!nativeeditor_status", e._ganttMode) === "true_deleted" && (s = !0, e.setUpdated(n, !1));
    for (let r = 0; r < a.length && !e._in_progress[n]; r++) a[r] === n && (i.getUserData(n, "!nativeeditor_status", e._ganttMode) === "inserted" && (s = !0), e.setUpdated(n, !1));
    return s;
  }
  handleResourceAssignmentCRUD(n, e) {
    ...
    function r(o) {
      const l = o.id;
      i.exists(l) && (n.setGanttMode("assignment"), n.setUpdated(l, !0, "inserted")), delete s[l];
    }
    e.attachEvent("onBeforeTaskAdd", function(o, l) {
      ...
      i.exists(o) && (s[o] ? r(l) : (n.setGanttMode("assignment"), n.setUpdated(o, !0)));
  ...
  handleResourceCRUD(n, e) {
    ...
    i.attachEvent("onAfterAdd", (a, s) => {
      ...
      n.setGanttMode("resource"), !this.clientSideDelete(a, n, e) && (n.storeItem(s), n.setUpdated(a, !0, "deleted"));
    });
  }
  handleBaselineCRUD(n, e) {
    ...
    i.attachEvent("onAfterAdd", (a, s) => {
      ...
      n.setGanttMode("baseline"), !this.clientSideDelete(a, n, e) && (n.storeItem(s), n.setUpdated(a, !0, "deleted"));
    });
  }
  detach() {
...
let ee = re, Vn = class {
  ...
  afterUpdateCallback(t, n, e, i, a) {
    ...
    this._in_progress[s] !== "wait" ? (r && this.$gantt.setUserData(t, this.action_param, "", this._ganttMode), delete this._in_progress[s]) : (delete this._in_progress[s], this.setUpdated(n, !0, this.$gantt.getUserData(t, this.action_param, this._ganttMode))), this.callEvent("onAfterUpdate", [o, e, n, i]);
  }
  afterUpdate(t, n, e) {
    let i;
...
function Fi(t) {
  ...
  function r(l, d, u) {
    ...
    return ht(d, function(g) {
      _[a(g)] = !0;
    }), n[h] ? c = n[h] : (c = n[h] = [], t.eachTask(function(g) {
      if (u) {
  ...
  const n = { renderEditableLabel: function(e, i, a, s, r) {
    ...
    t.attachEvent("onGanttReady", function() {
      ...
      t.event(t.$container, "keypress", function(s) {
        ...
        s.key === "Tab" && (e = Vt(t.$container).indexOf(document.activeElement), i(s), setTimeout(function() {
          var l;
...
function Vi(t) {
  ...
  }), t.getTaskBy = n.getTaskBy, t.getResourceAssignments = n.getResourceAssignments, t.config.resource_property = "owner_id", t.config.resource_store = "resource", t.config.resource_render_empty_cells = !1, t.templates.histogram_cell_class = function(a, s, r, o, l) {
...
function Ui(t) {
  ...
  var e = "auto", i = "singleValue", a = "valueArray", s = "resourceValueArray", r = "assignmentsArray", o = e, l = "fixedDates", d = "fixedDuration", u = "default";
  function c(p, k) {
...
const oe = class oe {
  constructor(n = null) {
    this.canParse = (e) => {
      ...
      const i = this._config.store, a = this._config.format, s = this._config.short;
      let r = this.transferUnits[i].toMinutes(e), o = a;
      ...
      if (this.canParse(e)) {
        ...
        for (let d = 0; d < e.length; d++) {
          const u = e[d];
          l.test(u) ? s = a : a = !0, (s || o === d) && (s || (i += u), r += this._getNumericValue(i), a = s = !1, i = ""), i += u;
        }
        if (r) {
...
const de = class de extends Ne {
  constructor(n, e) {
    super(e), this.format = (i) => {
      const a = this._getFormattedLinkType(this._getLinkTypeName(i.type)), s = this._getWBSCode(i.source), r = this._getLagString(i.lag);
      return i.type !== this._gantt.config.links.finish_to_start || i.lag ? `${s}${a}${r}` : s;
    }, this.parse = (i) => {
      if (!this.canParse(i)) return null;
      const a = this._linkReg.exec(i)[0].trim(), s = i.replace(a, "").trim(), r = this._findTypeFormat(a), o = this._getLinkTypeNumber(r);
...
const Ge = function(t, n) {
  ...
  let a = i || n.$task_data.scrollHeight, s = !1, r = !1;
...
}, Un = function(t, n, e, i, a, s) {
  const r = Ge(t, a), o = ne(t, n, a);
  let l = o.maxHeight, d = e.height, u = d > i, c = e.rowHeight >= i && !o.splitChild && !r;
  (u || c) && (d = i), l < d && (d = l);
    ...
    t.config.baselines && (t.attachEvent("onParse", function() {
      n.eachItem(function(a) {
        ...
        if (t.isTaskExists(s)) {
          ...
          o && r.baselines.push(a), q(t) || (pt(r) ? e(r) : t.adjustTaskHeightForBaselines(r));
        }
      });
...
class ea {
  ...
  attachEvents() {
    this._dnd.attachEvent("onBeforeDragStart", (n, e) => {
      ...
      if (this.$grid.callEvent("onColumnDragMove", [{ draggedColumn: s, targetColumn: r, draggedIndex: i, targetIndex: e }]) === !1) return this.cleanTargetMarker(), void this.$gantt.render();
  ...
  reorderColumns() {
    ...
    this.$grid.callEvent("onBeforeColumnReorder", [{ draggedColumn: a, targetColumn: s, draggedIndex: e, targetIndex: n }]) !== !1 && n !== e && (i.splice(e, 1), i.splice(n, 0, a), this.$gantt.render(), this.$grid.callEvent("onAfterColumnReorder", [{ draggedColumn: a, targetColumn: s, draggedIndex: e, targetIndex: n }]));
  }
  findColumnsIndexes() {
    ...
    for (let g = d; c(g, u) && (i === void 0 || a === void 0); g = h(g)) e[g].hide || (o.startX = o.endX, o.endX += e[g].width, _ >= o.startX && (_ <= o.endX || !c(h(g), u)) && (i = g, s = o.startX, r = o.endX, l = (_ - o.startX) / (o.endX - o.startX)), n === e[g].name && (a = g));
...
Gt.prototype = { init: function(t) {
  var n = this.$gantt, e = n._waiAria.gridAttrString(), i = n._waiAria.gridDataAttrString(), a = this.$getConfig(), s = a.reorder_grid_columns || !1;
  ...
  var l = function(u, c) {
    var h = { column_before_start: u.bind(function(_, g, y) {
      ...
      var m = c.$getConfig(), b = _.config, v = c.getGridColumns(), f = parseInt(b.drag_index, 10), p = v[f], k = Y(c.$grid_scale), x = parseInt(b.marker.style.left, 10), $ = p.min_width ? p.min_width : m.min_grid_column_width, w = c.$grid_data.offsetWidth, T = 0, S = 0;
  ...
  var d = function(u, c) {
    var h = { row_before_start: u.bind(function(_, g, y) {
      ...
      var m = c.$config.rowStore, b = c.$getConfig(), v = _.config, f = v.drag_id, p = c.getItemHeight(f), k = c.getItemTop(f) - g.scrollTop, x = Y(c.$grid_data), $ = parseInt(v.marker.style.top, 10), w = k + x.y, T = 0, S = b.min_task_grid_row_height;
  ...
  d.init(), this._addLayers(this.$gantt), this._initEvents(), s && (this._columnDND = new ea(n, this), this._columnDND.init()), this.callEvent("onReady", []);
  ...
  this._mouseDelegates.delegate("click", "gantt_close", t.bind(function(n, e, i) {
    ...
    if (!a) return !0;
    var s = et(n, this.$config.item_attribute);
  ...
  for (a -= d, r = n + 1; r < i.length; r++) {
    var u = i[r], c = Math.round(l * (u.width / a));
  ...
  for (var h = l > 0 ? 1 : -1; l > 0 && h === 1 || l < 0 && h === -1; ) {
    ...
    for (r = n + 1; r < i.length; r++) {
      var g;
      if ((g = i[r].width + h) == this._getColumnWidth(i[r], e, g) && (l -= h, i[r].width = g), !l) break;
    }
    if (_ == l) break;
  }
  return l && n > -1 && (g = i[n].width + l) == this._getColumnWidth(i[n], e, g) && (i[n].width = g), this._getColsTotalWidth();
  ...
  if (t.autofit || i.length) {
    ...
    if (t.autofit && !t.grid_elastic_columns) for (s = 0; s < a.length; s++) {
      ...
      a[s] += d, (u = this._getColumnWidth(n[s], t, a[s])) != a[s] && (d = u - a[s], a[s] = u), l -= d;
    }
  ...
  for (var u = 0; u < a.length; u++) {
    ...
    var y = t._sort && h.name == t._sort.name ? `<div data-column-id="${h.name}" class="gantt_sort gantt_${t._sort.direction}"></div>` : "", m = ["gantt_grid_head_cell", "gantt_grid_head_" + h.name, c ? "gantt_last_cell" : "", i.grid_header_class(h.name, h)].join(" "), b = "width:" + (_ - (c ? 1 : 0)) + "px;", v = h.label || o["column_" + h.name] || o[h.name];
...
Yn.prototype = { _getIntervals: function(t) {
  ...
  const e = J(t.getConfig()), i = J(n.getConfig()), a = e.parsed, s = i.parsed;
...
function Re(t, n) {
  this.argumentsHelper = n, this.$gantt = t, this._workingUnitsCache = typeof Map < "u" ? new aa() : new sa(), this._largeUnitsCache = new ra(this), this._dateDurationCache = new oa(), this._worktime = null, this._cached_timestamps = {}, this._cached_timestamps_count = 0;
}
Re.prototype = { units: ["year", "month", "week", "day", "hour", "minute"], _clearCaches: function() {
  ...
  for (var _ = !1; s.valueOf() < r.valueOf(); ) {
    if (_ = (u = this._nextDate(s, e, i)).valueOf() > r.valueOf(), this._isWorkTime(s, e)) (c || h && _) && (o = a[e + "_start"](new Date(s)), l = a.add(o, i, e)), c ? (c = !1, u = this._nextDate(o, e, i), d += (l.valueOf() - s.valueOf()) / (l.valueOf() - o.valueOf())) : h && _ ? (h = !1, d += (r.valueOf() - s.valueOf()) / (l.valueOf() - o.valueOf())) : d++;
    else {
  ...
  var l = this.$gantt.date.day_start(new Date(s)), d = s, u = this._getMinutesBetweenSingleDay(r, o, e, i) / 60, c = this._getMinutesBetweenSingleDay(l, d, e, i) / 60, h = u + this._getWorkUnitsForRange(o, l, e, i) + c;
  return Math.round(h);
}, getConfig: function() {
  ...
  if (t.customWeeks) {
    ...
    for (var n in t.customWeeks) {
      ...
      if (a.from && a.to) {
        var s = a.from, r = a.to;
        (!e || e > s.valueOf()) && (e = s.valueOf()), (!i || i < r.valueOf()) && (i = r.valueOf()), t.parsed.customWeeksBoundaries.push({ from: s.valueOf(), fromReadable: new Date(s), to: r.valueOf(), toReadable: new Date(r), name: n }), t.parsed.haveCustomWeeks = !0;
  ...
  for (a = a || 1; s.valueOf() < r.valueOf(); ) {
    if (this._isWorkTime(s, i)) return !0;
  ...
  var g = e;
  return i === n && (g = new Date(t.getFullYear(), t.getMonth(), t.getDate(), 0, 0, r)), { added: i, end: g };
  ...
  for (var o = 0, l = [], d = 0; a < n; ) {
    ...
    var _ = n - a, g = this._getTimeOfDayStamp(i, c);
    if (l.length && d) if (l[l.length - 1].end <= g && _ > d) a += d, i = this.$gantt.date.add(i, -1, "day");
    else {
      for (var y = !1, m = null, b = null, v = l.length - 1; v >= 0; v--) if (l[v].start < g - 1 && l[v].end >= g - 1) {
        y = !0, m = l[v], b = l[v - 1];
        break;
      }
      if (y) if (g === m.end && _ >= m.durationMinutes) a += m.durationMinutes, i = this.$gantt.date.add(i, -m.durationMinutes, "minute");
      else if (!s && _ <= g / 60 - m.startMinute) a += _, i = this.$gantt.date.add(i, -_, "minute");
      else if (s) _ <= g / 60 - m.startMinute ? (a += _, i = this.$gantt.date.add(i, -_, "minute")) : (a += g / 60 - m.startMinute, i = b ? new Date(i.getFullYear(), i.getMonth(), i.getDate(), 0, 0, b.end) : this.$gantt.date.day_start(i));
      else {
        var f = this._getMinutesPerHour(i);
        f <= _ ? (a += f, i = this._nextDate(i, "hour", e)) : (r = this._subtractMinutesUntilHourStart(i, _), a += r.added, i = r.end);
      }
  ...
  if (a < n) {
    var $ = n - a;
    r = this._subtractMinutesUntilHourStart(i, $), a += r.added, i = r.end;
  }
  return i;
  ...
  for (var r = 0, o = [], l = 0, d = this._isMinutePrecision(i); a < n; ) {
    ...
    u !== r && (o = this._getWorkHours(i), l = this._getMinutesPerDay(i), r = u);
    ...
    if (o.length && l) if (o[0].start >= h && c >= l) {
      ...
      if (_) if (h === g.start && c >= g.durationMinutes) a += g.durationMinutes, i = this.$gantt.date.add(i, g.durationMinutes, "minute");
      else if (c <= g.endMinute - h / 60) a += c, i = this.$gantt.date.add(i, c, "minute");
      else {
        var m = this._getMinutesPerHour(i);
        m <= c ? (a += m, i = d ? this.$gantt.date.add(i, m, "minute") : this._nextDate(i, "hour", e)) : (a += (s = this._addMinutesUntilHourEnd(i, c)).added, i = s.end);
      }
  ...
  var i = new Date(t), a = null, s = !1;
  this._getWorkHours(i).length || (i = this._getClosestWorkTime(i, "day", n < 0 ? "past" : "future"), n < 0 && (i = new Date(i.valueOf() - 1), s = !0), e = this._getWorkHours(i));
...
function ca(t) {
  ...
  function a(r, o, l, d) {
    ...
    let h;
    h = r.type === t.config.types.project && r.auto_scheduling === !1 ? c : u, o.$no_start && (r[h.start] = l ? new Date(l) : n(r, this.getParent(r))), o.$no_end && (r[h.end] = d ? new Date(d) : this.calculateEndDate({ start_date: r[h.start], duration: this.config.duration_step, task: r })), (o.$no_start || o.$no_end) && this._init_task_timing(r);
  }
  function s(r) {
  ...
  t._init_task_timing = function(r) {
    ...
    r.$effective_calendar && r.$effective_calendar !== u.id && (i(r), this.config.inherit_calendar && this.isSummaryTask(r) && this.eachTask(function(c) {
      i(c);
...
function ua(t) {
  ...
  var i = (function() {
    ...
    q(t) || (this.$root.innerHTML = "", this.$root.gantt = this, Ie(this), this.config.layout.id = "main", this.$layout = this.$ui.createView("layout", this.$root, this.config.layout), this.$layout.attachEvent("onBeforeResize", function() {
      ...
      r.className = "gantt_container_resize_watcher", r.tabIndex = -1, s.config.wai_aria_attributes && (r.setAttribute("role", "none"), r.setAttribute("aria-hidden", !0)), s.env.isSalesforce && (s.config.container_resize_method = "timeout"), s.$root.appendChild(r), r.contentWindow ? _n(s, r.contentWindow) : (s.$root.removeChild(r), _n(s, window));
    }(this));
  }).bind(t);
  t.resetLayout = function() {
    i(), a(), this.render();
  }, t._reinit = function(s) {
    this.callEvent("onBeforeGanttReady", []), this._update_flags(), this.$services.getService("templateLoader").initTemplates(this), i(), this.$root = null, s && (this.$root = We(s), a(), this.$mouseEvents.reset(this.$root), function(r) {
...
function Ua() {
  this.constants = Ci, this.version = "9.1.0", this.license = "individual", this.templates = {}, this.ext = {}, this.keys = { edit_save: this.constants.KEY_CODES.ENTER, edit_cancel: this.constants.KEY_CODES.ESC };
}
function Ga(t) {
  ...
  n.plugins = function(l) {
    ...
    return { cache: !0, method: "get", parse: function(d) {
      ...
      return new l.Promise(function(y, m) {
        var b = typeof XMLHttpRequest !== void 0 ? new XMLHttpRequest() : new tt.ActiveXObject("Microsoft.XMLHTTP"), v = navigator.userAgent.match(/AppleWebKit/) !== null && navigator.userAgent.match(/Qt/) !== null && navigator.userAgent.match(/Safari/) !== null;
        h && (b.onreadystatechange = function() {
          if (b.readyState == 4 || v && b.readyState == 3) {
            ...
            setTimeout(function() {
              typeof _ == "function" && _.apply(tt, [{ xmlDoc: b, filePath: u }]), y(b), typeof _ == "function" && (_ = null, b = null);
            }, 0);
          }
        });
  ...
  }(n), n.date = Ai(n), n.RemoteEvents = Di;
  var a = function(l) {
    function d(c) {
  ...
  var o = new Ma({ en: ya, ar: ha, be: _a, ca: ga, cn: fa, cs: pa, da: ma, de: va, el: ka, es: ba, fa: xa, fi: $a, fr: wa, he: Sa, hr: Ta, hu: Ca, id: Ea, it: Aa, jp: Da, kr: Ia, nb: La, nl: Na, no: Pa, pl: Ra, pt: Ha, ro: Oa, ru: Ba, si: za, sk: ja, sv: Fa, tr: Wa, ua: Va });
...
function qa(t) {
  ...
  y.hide = function(v) {
    for (; v && v.getAttribute && !v.getAttribute(n); ) v = v.parentNode;
  ...
  var m = function(v, f, p, k) {
    switch ((v = g.apply(this, arguments)).type = v.type || "info", v.type.split("-")[0]) {
      ...
      default:
        return function(x) {
          m.area || (m.area = document.createElement("div"), m.area.className = "gantt_message_area", m.area.style[m.position] = "5px"), Z(m.area, document.body) || document.body.appendChild(m.area), m.hide(x.id);
...
const Ja = function(t) {
  ...
  function i(a, s, r, o) {
    ...
    a != "resizer" || r.mode || (o.$config.cols ? r.mode = "x" : r.mode = "y"), a != "viewcell" || r.view != "scrollbar" || r.scroll || (o.$config.cols ? r.scroll = "y" : r.scroll = "x"), (r = J(r)).id || e[r.view] || (r.id = r.view), r.id && !r.css && (r.css = r.id + "_cell");
    var d = new l.create(s, r, this, t);
    return l.configure && l.configure(d), Ya(d, o), d.$id || (d.$id = r.id || t.uid()), d.$parent || typeof s != "object" || (d.$parent = s), d.$config || (d.$config = r), e[d.$id] && (d.$id = t.uid()), e[d.$id] = d, d;
  }
...
var Ka = /* @__PURE__ */ function(t) {
  return function(n) {
    ...
    function a(h) {
      h = h || window.event;
      var _ = n.locate(h), g = r(h, e.click), y = !0;
    ...
    function s(h) {
      var _ = (h = h || window.event).target || h.srcElement, g = n.locate(_), y = n.locate(_, n.config.link_attribute), m = !n.checkEvent("onContextMenu") || n.callEvent("onContextMenu", [g, y, h]);
    ...
    function l(h) {
      h = h || window.event;
      var _ = n.locate(h), g = r(h, e.doubleclick), y = !n.checkEvent("onTaskDblClick") || _ === null || n.callEvent("onTaskDblClick", [_, h]);
      if (y) {
        if (!o(g, h, _)) return;
    ...
    return { reset: c, global: function(h, _, g) {
      i(h, _, g, null);
    }, delegate: i, detach: function(h, _, g, y) {
      if (e[h] && e[h][_]) {
...
var Za = function(t) {
  var n = /* @__PURE__ */ function(e) {
    ...
    function r(o, l, d) {
      ...
      typeof l.renderer == "function" ? (u = l.renderer, h = ie) : (u = l.renderer.render, c = l.renderer.update, _ = l.renderer.onrender, l.renderer.isInViewPort ? g = l.renderer.isInViewPort : h = l.renderer.getRectangle, h || h === null || (h = ie));
      var y = l.filter;
      return d && d.setAttribute(e.config.layer_attribute, !0), a[o] = { render_item: function(m, b, v, f, p) {
        if (b = b || d, !y || y(m)) {
          ...
          var w = null;
          !Ft(e) && (h || g) && $ ? (g ? g(m, $, k, x, e) : pn($, h(m, k, x, e))) && (w = u.call(e, m, k, x, $)) : w = u.call(e, m, k, x, $), this.append(m, w, b);
          var T = b.nodeType == 11;
          _ && !T && w && _.call(e, m, w, k);
        } else this.remove_item(m.id);
      }, clear: function(m) {
        this.rendered = i[o] = {}, l.append || this.clear_container(m);
      }, clear_container: function(m) {
        (m = m || d) && (m.innerHTML = "");
      }, get_visible_range: function(m) {
        var b, v, f = s(l), p = f ? f.$getConfig() : null;
        return p && p.smart_rendering && (b = f.getViewPort()), f && b && (typeof l.renderer == "function" ? v = Q(e, f, 0, m, b) : l.renderer && l.renderer.getVisibleRange && (v = l.renderer.getVisibleRange(e, f, p, m, b))), v || (v = { start: 0, end: m.count() }), v;
        ...
        if (v && v.$getConfig().smart_rendering && !Ft(e) && this.rendered && (h || g)) {
          ...
          for (var S = {}, C = (T = 0, m.length); T < C; T++) {
            ...
            w[E.id] = !1, A && A.parentNode ? (g ? g(E, k, v, f, e) : pn(k, h(E, v, f, e))) ? (c && c.call(e, E, A, v, f, k), this.restore(E, p)) : w[E.id] = !0 : (S[m[T].id] = !0, this.render_item(m[T], p, k, v, f));
          }
  ...
  return { createGroup: function(e, i, a, s) {
    var r = { tempCollection: [], renderers: {}, container: e, filters: [], getLayers: function() {
      ...
      for (let u = 0; u < d.length; u++) {
        ...
        this.renderers[h] = n.getRenderer(h, o, c), s && s(o, t), this.tempCollection.splice(u, 1), u--;
      }
    }, addLayer: function(o) {
      if (o) {
  ...
  function i(s, r, o) {
    r.$getConfig(), s.getVisibleItems().forEach(function(l) {
      var d = function(u, c, h, _) {
        ...
        var g = vn(_.getTask(u.source), c), y = vn(_.getTask(u.target), c);
        if (!g || !y) return null;
...
function Zn(t, n, e, i, a) {
  ...
  return _ > g && (y = g, g = _, _ = y), c += -100, h += 100, _ += -100, g += 100, !(n.x > h && n.x > g) && !(n.x_end < c && n.x_end < _);
}
function Qa(t, n) {
  if (t.view) {
    var e = t.view;
...
var wt = function() {
  ...
  return t.prototype.destructor = function() {
    ...
    e === document.body && (i = document.body.offsetWidth, a = document.body.offsetHeight), i < s.minWidth && (i = s.minWidth), i > s.maxWidth && (i = s.maxWidth), a < s.minHeight && (a = s.minHeight), a > s.maxHeight && (a = s.maxHeight), this.setSize(i, a), this.$preResize, this.$preResize = !1;
    ...
    typeof n == "string" && (n = [n]);
    var s = {};
    for (a = 0; a < n.length; a++) $t(i, n[a]), s[n[a]] = !0;
    e._currentBorders = s;
...
var Qn = function(t) {
  function n(e, i, a) {
    ...
    return e && (s.$root = !0), s._parseConfig(i), s.$name = "layout", s;
  }
  return F(n, t), n.prototype.destructor = function() {
    ...
    for (var u, c = 0; c < i.length; c++) e[(u = i[c]).$config.scroll] ? d(u) : u.shouldHide() ? o.push(u) : u.shouldShow() ? l(u) : u.isVisible() ? s.push(u) : r.push(u);
    var h = {};
    ...
    return e.forEach((a) => {
      ...
      s.id = a.$id, s.visible = r, s.boundViews = c, o === "x" ? (s.x = l, s.x_inner = d, s.x_pos = u || 0) : (s.y = l, s.y_inner = d, s.y_pos = u || 0), i.push(s);
    }), i;
    ...
    var e = An(this.$container, this.$toHTML());
    this.$fill(e, null);
    const i = this.$gantt;
    this._eachChild((a) => {
    ...
    for (var a = In(e, "gantt_layout_cell"), s = a.length - 1; s >= 0; s--) {
      var r = this.$cells[s];
    ...
    for (s = 0; s < i.length; s++) if (!i[s].$config.hidden) {
      var r = s >= a, o = "";
      !r && i[s + 1] && i[s + 1].$config.view == "scrollbar" && (this._xLayout ? r = !0 : o = "gantt_layout_cell_border_transparent"), this._setBorders(r ? [] : [e, o], i[s]);
    }
  }, n.prototype._updateCellVisibility = function() {
    for (var e = this._visibleCells || {}, i = !this._visibleCells, a = {}, s = null, r = [], o = 0; o < this._sizes.length; o++) (s = this.$cells[o]).$config.hide_empty && r.push(s), !i && s.$config.hidden && e[s.$id] ? s._hide(!0) : s.$config.hidden || e[s.$id] || s._hide(!1), s.$config.hidden || (a[s.$id] = !0);
    ...
    for (o = 0; o < this.$cells.length; o++) if (!(a = this.$cells[o]).$config.hidden) {
      ...
      this._xLayout ? (_ = this._calcSize(s.width, s, !0), g = i - 2 * r) : (_ = e - 2 * r, g = this._calcSize(s.height, s, !1)), a.setSize(_, g);
    }
  }, n;
}(wt), ts = function(t) {
  function n(e, i, a) {
  ...
  function n(e, i, a) {
    ...
    if (i.view) {
      i.id && (this.$id = ut());
      var r = J(i);
      if (delete r.config, delete r.templates, this.$content = this.$factory.createView(i.view, this, r, this), !this.$content) return !1;
    }
  ...
  return F(n, t), n.prototype.init = function() {
    ...
    var a, s, r, o, l;
    this._xMode ? (a = e - this._positions.x, s = this._behind.$config.width || this._behind.$view.offsetWidth, o = this._front.$config.width || this._front.$view.offsetWidth, r = this._behind.$config.minWidth, l = this._front.$config.minWidth) : (a = i - this._positions.y, s = this._behind.$config.height || this._behind.$view.offsetHeight, o = this._front.$config.height || this._front.$view.offsetHeight, r = this._front.$config.minHeight, l = this._front.$config.minHeight);
    ...
    for (var a = this.$factory.getView("main"), s = this._getGroupName(), r = a.getCellsByType("resizer"), o = 0; o < r.length; o++) r[o]._getGroupName() == s && r[o] != this && e.call(i || this, r[o]);
    ...
    this._eachGroupItem(function(l) {
      ...
      this._xMode ? d.x += l._behind.$config.width - this._behind.$config.width : d.y += l._behind.$config.height - this._behind.$config.height, l._positions = d, r.push(l._getResizePosition(e, i));
    });
    ...
    var i = this._xMode ? "offsetWidth" : "offsetHeight", a = this._xMode ? this._positions.nextX : this._positions.nextY, s = this._front.$view[i], r = this._behind.$view[i], o = (s - a) / s * this._front.getSize().gravity, l = (r + a) / r * this._behind.getSize().gravity;
  ...
  return F(e, t), e.prototype.init = function(a) {
    ...
    if (this._checkWheelTarget(s)) {
      ...
      if (y !== !1 && n.indexOf(y) >= 0 && (!a[y] || a.deltaX || a.wheelDeltaX || (_ = 2 * g, g = 0)), _ && Math.abs(_) > Math.abs(g)) {
...
function he(t) {
  ...
  return { init: function() {
    ...
    } else this.previousDragId = i.drag_id, s = this._getIdsFromAssignments(this.previousDragId);
    else {
    ...
    const a = this.$gantt, s = [], r = a.getTask(i);
        ...
        if (d.$role != "task") {
          ...
          u.sort(function(h, _) {
            ...
            if (!c[h.task_id] && i.isTaskExists(h.task_id)) {
              ...
              (_ = a.$config.copyOnParse ? i.copy(g) : Object.create(g)).id = g.id + "_" + d.id, _.$task_id = g.id, _.$resource_id = d.id, _[a.$parentProperty] = d.id, _.$role = "task", s.push(_), r[_.id] = !0;
            }
          });
        } else o[d.id] = !0;
...
Ye.prototype = { init: function(t) {
  ...
  const e = this.$gantt, i = function(s, r) {
    return r.type === e.config.types.project && r.auto_scheduling === !1;
  };
  var a = [{ expose: !0, renderer: this.$gantt.$ui.layers.taskBar(), container: this.$task_bars, filter: [n, function(s, r) {
    return !r.hide_bar;
  }, function(s, r) {
    return !i(0, r);
  ...
  var t = this, n = ss(), e = t.$config.rowStore;
  ...
  var i = It(this._tasks.left, t), a = this._tasks.left[i], s = e.width[i] || e.col_width, r = 0;
  s && (r = (t - a) / s, e.rtl && (r = 1 - r));
  const o = (n ? n.calendar : null) || null;
  var l = 0;
  r && (l = this._getColumnDuration(e, e.trace_x[i]));
  const d = e.trace_x[i], u = new Date(e.trace_x[i].valueOf() + Math.round(r * l));
  if (!o) return u;
  ...
  var s = It(e, n), r = this._getClosestVisibleColumn(s, e, i), o = e[r], l = this._tasks.trace_index_transition;
  if (!o) return l ? l[0] : 0;
...
var rs = function(t) {
  ...
  return F(n, t), H(n.prototype, { init: function() {
    ...
    var e = this.$gantt;
    t.prototype._initEvents.apply(this, arguments), this._mouseDelegates.delegate("click", "gantt_row", e.bind(function(i, a, s) {
...
const ds = { init: function(t, n) {
  ...
  if (i.ext && i.ext.keyboardNavigation && i.ext.keyboardNavigation.attachEvent("onKeyDown", function(a, s) {
    var r = i.constants.KEY_CODES, o = !1;
    ...
    switch (h) {
      ...
      default:
        if (t.isVisible()) _ = !0;
          ...
          y.alt || m && s.getCommandHandler(r, "taskCell") || d && !t.isVisible() && (e.startEdit(l.id, l.columnName), _ = !0);
        }
    }
    return !_;
  });
...
function us(t) {
  ...
  return F(e, n), H(e.prototype, { show: function(i, a, s, r) {
...
function gs(t) {
  ...
  return F(s, n), H(s.prototype, { show: function(r, o, l, d) {
...
function ps(t) {
  ...
  return F(e, n), H(e.prototype, { show: function(a, s, r, o) {
...
function vs(t) {
  t.config.editor_types = { text: new (us())(), number: new (hs())(), select: new (_s())(), date: new (gs(t))(), predecessor: new (fs(t))(), duration: new (ps(t))() };
}
function ks(t) {
  var n = /* @__PURE__ */ function(a) {
    ...
    return { setMapping: function(r) {
      ...
      return s || (a.config.keyboard_navigation_cells && a.ext.keyboardNavigation ? cs : ds);
    } };
  }(t), e = {};
  _t(e);
  var i = { init: vs, createEditors: function(a) {
    function s(c, h) {
    ...
    var r = ms(t), o = [], l = [], d = null, u = { _itemId: null, _columnName: null, _editor: null, _editorType: null, _placeholder: null, locateCell: function(c) {
...
function xs(t, n, e, i, a) {
  if (!a.isSplitTask(t)) return !1;
  var s = a.getSubtaskDates(t.id);
  return Mt({ id: t.id, start_date: s.start_date, end_date: s.end_date, parent: t.parent }, n, e);
}
function Oe(t, n, e) {
  return { top: n.getItemTop(t.id), height: n.getItemHeight(t.id), left: 0, right: 1 / 0 };
}
...
function ws(t) {
  var n = { current_pos: null, dirs: { left: "left", right: "right", up: "up", down: "down" }, path: [], clear: function() {
    ...
    var l = this.get_endpoint(a, s, r, o), d = t.config, u = l.e_y - l.y, c = l.e_x - l.x, h = n.dirs, _ = s.getItemHeight(a.source);
    ...
    var l = s.$getConfig(), d = this.get_line_type(a, l), u = d.from_start, c = d.to_start, h = i(r, s, l), _ = i(o, s, l);
  ...
  function i(a, s, r) {
    ...
    let l = ne(t, s, a), d = l.maxHeight, u = l.splitChild;
  ...
  return { render: function(a, s, r) {
    ...
    const h = function(m, b) {
      ...
      return p;
    }(n.get_lines(c, s).filter((m) => m.size > 0), r), _ = function(m, b, v, f) {
      const p = document.createElement("div");
      return m.forEach((k) => {
        let x;
        k.type === "line" ? x = n.render_line(k.data, null, b, v.source) : k.type === "corner" ? x = n.render_corner(k.data, b) : k.type === "arrow" && (x = n.render_arrow(k.data, f)), p.appendChild(x);
      }), p;
    }(h, s, a, r);
    ...
    return o = n[r] ? n[r] : n[r] = function(l, d, u, c) {
      ...
      t.config.process_resource_assignments && d === t.config.resource_property ? (h = l.$role == "task" ? t.getResourceAssignments(l.$resource_id, l.$task_id) : t.getResourceAssignments(l.id), _ = !0) : h = l.$role == "task" ? [] : t.getTaskBy(d, l.id), g = function(w, T, S) {
        ...
        for (let M = 0; M < w.length; M++) {
          ...
          for (D[z.id] = {}; B < P; ) {
            const K = D[z.id], W = B.valueOf();
            B = t.date.add(B, E, C), K[W] !== !1 && (A[W] || (A[W] = { tasks: [], assignments: [] }), A[W].tasks.push(L), S && A[W].assignments.push(I));
          }
        }
        return A;
      }(h, u, _);
...
function Ts(t, n, e, i) {
  var a = 100 * (1 - (1 * t || 0)), s = i.posFromDate(n), r = i.posFromDate(e), o = document.createElement("div");
  return o.className = "gantt_histogram_hor_bar", o.style.top = a + "%", o.style.left = s + "px", o.style.width = r - s + 1 + "px", o;
}
function Cs(t, n, e) {
  if (t === n) return null;
  var i = 1 - Math.max(t, n), a = Math.abs(t - n), s = document.createElement("div");
  return s.className = "gantt_histogram_vert_bar", s.style.top = 100 * i + "%", s.style.height = 100 * a + "%", s.style.left = e + "px", s;
}
function Es(t) {
  var n = ei(t), e = {}, i = {}, a = {};
  function s(l, d) {
  ...
  function r(l, d, u, c, h, _, g) {
    ...
    var m = function(b, v, f, p) {
      for (var k = v.getScale(), x = document.createElement("div"), $ = Ct(k, p), w = $.start; w <= $.end; w++) {
        ...
        if (E < 0) return null;
        var A = Math.min(b[S.valueOf()] / f, 1) || 0, D = Ts(E, T, S, v);
        D && x.appendChild(D);
        var M = Cs(E, A, v.posFromDate(S));
        M && x.appendChild(M);
      }
      return x;
    }(u, h, _, g);
  ...
  return { render: function(l, d, u, c) {
    var h = d.$getTemplates(), _ = d.getScale(), g = n(l, u.resource_property, _, d), y = [], m = {}, b = l.capacity || d.$config.capacity || 24;
    ...
    var _ = u.$getTemplates(), g = u.getScale(), y = n(l, c.resource_property, g, u), m = l.capacity || u.$config.capacity || 24, b = {}, v = !!h, f = Ct(g, h), p = {};
...
function Ds(t) {
  ...
  function e(i, a, s) {
    ...
    let { height: d, marginTop: u } = Un(t, a, l, 30, i, s), c = d, h = 0;
    switch (r) {
      case "snet":
      case "fnet":
      case "mso":
    ...
    switch (i.type === t.config.types.milestone && (u -= 1), o.style.height = d + "px", o.style.width = c + "px", o.style.left = l.left + "px", o.style.top = l.top + "px", o.style.marginLeft = h + "px", o.style.marginTop = u + "px", o.className = "gantt_constraint_marker gantt_constraint_marker_" + r, r) {
      case "snet":
      case "snlt":
      case "fnet":
      case "fnlt":
...
`;
  ...
  return { render: function(i, a, s, r) {
    if (ni(t)) {
  ...
  }, isInViewPort: As, getVisibleRange: Q };
}
function Is(t, n, e, i, a) {
  return Je(t, n, e, 0, a, { start_date: "deadline", end_date: "deadline" });
}
function Ms(t, n, e, i, a) {
  let s = !1;
  const r = { start_date: "start_date", end_date: "end_date" };
  return t.type == a.config.types.milestone && (r.end_date = r.start_date), t.baselines && (s = kn(t, n, e, i, a, r)), pt(t) && a.eachTask(function(o) {
    s || o.baselines && o.baselines.length && (o.type == a.config.types.milestone && (r.end_date = r.start_date), kn(o, n, e, i, a, r) && (s = !0));
  }, t.id), s;
}
function kn(t, n, e, i, a, s) {
  for (var r = 0; r < t.baselines.length; r++)
    if (Je({ id: t.id, parent: t.parent, start_date: t.baselines[r].start_date, end_date: t.baselines[r].end_date }, n, e, 0, a, s)) return !0;
}
const Ls = { init: function(t, n) {
  ...
  if (n.$config.bind && t.getDatastore(n.$config.bind)) {
    ...
    t.defined(n.$getConfig().dnd_sensitivity) && (i.config.sensitivity = n.$getConfig().dnd_sensitivity), i.attachEvent("onBeforeDragStart", t.bind(function(o, l) {
      ...
      var c = s(), h = c.getItem(i.config.id);
      ...
      var m = g.getItem(i.config.id), b = i._getTargetY(l), v = i._getTaskByY(b, g.getIndexById(m.id));
      function f(A, D) {
        return !g.isChildOf(p.id, D.id) && (A.$level == D.$level || _.order_branch_free);
      }
      if (g.exists(v) || (v = i.lastTaskOfLevel(_.order_branch_free ? m.$level : 0)) == i.config.id && (v = null), g.exists(v)) {
        var p = g.getItem(v), k = n.getItemTop(p.id), x = n.getItemHeight(p.id);
        if (k + x / 2 < b) {
...
function ii(t, n, e, i, a) {
  ...
  return i.exists(s) ? i.calculateItemLevel(i.getItem(s)) === e ? a ? nt.nextSiblingTarget(t, s, i) : nt.prevSiblingTarget(t, s, i) : nt.firstChildTarget(t, s, i) : null;
}
function $e(t, n, e, i) {
  return ii(t, n, e, i, !0);
}
...
function bn(t, n, e, i, a, s) {
  var r;
  if (n !== a.$getRootId()) {
    var o = a.getItem(n), l = a.calculateItemLevel(o);
    if (l === s) {
      var d = a.getPrevSibling(n);
      e < 0.5 && !d ? r = nt.prevSiblingTarget(t, n, a) : (e < 0.5 && (n = d), r = nt.nextSiblingTarget(t, n, a));
    } else if (l > s) a.eachParent(function(g) {
      a.calculateItemLevel(g) === s && (n = g.id);
    }, o), r = $e(t, n, s, a);
    else {
      var u = $e(t, n, s, a), c = yn(t, n, s, a);
      r = e < 0.5 ? u : c;
    }
  } else {
    var h = a.$getRootId(), _ = a.getChildren(h);
    r = nt.createDropTargetObject(), r = _.length && i >= 0 ? $e(t, function(g) {
...
const $n = { removeLineHighlight: function(t) {
  ...
  var i = function(s, r) {
    var o = Y(r.$grid_data), l = dt(s, r.$grid_data), d = o.x + r.$grid.scrollLeft, u = l.y - 10, c = r.getItemHeight(s.targetId);
    u < o.y && (u = o.y);
  ...
  a || ((a = document.createElement("div")).className = "gantt_drag_marker gantt_grid_dnd_marker", a.innerHTML = "<div class='gantt_grid_dnd_marker_line'></div>", a.style.pointerEvents = "none"), t.child ? function(s, r, o) {
    var l = s.targetParent, d = xn({ x: 0, y: o.getItemTop(l) }, o), u = o.$grid_data.getBoundingClientRect().bottom + window.scrollY;
    ...
    var l = function(c, h) {
      ...
      return g.x = v + b * m, g.width = we(h.$gantt, Math.max(h.$grid_data.offsetWidth - g.x, 0), g.x), xn(g, h);
  ...
  if (n.$config.bind && t.getDatastore(n.$config.bind)) {
    ...
    t.defined(n.$getConfig().dnd_sensitivity) && (i.config.sensitivity = n.$getConfig().dnd_sensitivity), i.attachEvent("onBeforeDragStart", t.bind(function(o, l) {
      ...
      return !t.isReadonly(c) && !s(u) && (i.config.initial_open_state = c.$open, !!t.callEvent("onRowDragStart", [u, l.target || l.srcElement, l]) && void 0);
    }, t)), i.attachEvent("onAfterDragStart", t.bind(function(o, l) {
      var d = a(l);
      i.config.marker.innerHTML = d.outerHTML;
      var u = i.config.marker.firstChild;
      ...
      var d = r(l);
      return d && t.callEvent("onBeforeRowDragMove", [i.config.id, d.targetParent, d.targetIndex]) !== !1 || (d = nt.createDropTargetObject(i.config.drop_target)), $n.highlightPosition(d, i.config, n), i.config.drop_target = d, t._waiAria.reorderMarkerAttr(i.config.marker), this.callEvent("onRowDragMove", [i.config.id, d.targetParent, d.targetIndex]), !0;
    }, t)), i.attachEvent("onDragEnd", t.bind(function() {
      var o = n.$config.rowStore, l = o.getItem(i.config.id);
      $n.removeLineHighlight(i.config), l.$transparent = !1, l.$open = i.config.initial_open_state;
  ...
  function r(o) {
    var l, d = function(g) {
      var y = dt(g, n.$grid_data).y, m = n.$config.rowStore;
      document.doctype || (y += window.scrollY), y = y || 0;
      var b = n.$state.scrollTop || 0, v = t.$grid_data.getBoundingClientRect().height + b + window.scrollY, f = b, p = n.getItemIndexByTopPosition(n.$state.scrollTop);
      if (m.exists(p) || (p = m.countVisible() - 1), p < 0) return m.$getRootId();
      var k = m.getIdByIndex(p), x = n.$state.scrollTop / n.getItemHeight(k), $ = x - Math.floor(x);
      $ > 0.1 && $ < 0.9 && (v -= n.getItemHeight(k) * $, f += n.getItemHeight(k) * (1 - $));
      const w = Y(n.$grid_data), T = w.y + w.height, S = i.config.marker.offsetHeight;
      y + S + window.scrollY >= v && (i.config.marker.style.top = T - S + "px"), y >= v ? y = v : y <= f && (y = f, i.config.marker.style.top = w.y + "px");
    ...
    return document.doctype || (_ += window.scrollY), d !== c.$getRootId() && (u = (_ - n.getItemTop(d)) / n.getItemHeight(d)), h ? (l = bn(i.config.id, d, u, _, c, i.config.level)) && l.targetParent && s(l.targetParent) && (d = c.getPrevSibling(l.targetParent), l = bn(i.config.id, d, u, _, c, i.config.level)) : l = function(g, y, m, b, v) {
      var f;
      if (y !== v.$getRootId()) f = m < 0.25 ? nt.prevSiblingTarget(g, y, v) : !(m > 0.6) || v.hasChild(y) && v.getItem(y).$open ? nt.firstChildTarget(g, y, v) : nt.nextSiblingTarget(g, y, v);
      else {
        var p = v.$getRootId();
        f = v.hasChild(p) && b >= 0 ? nt.lastChildTarget(g, p, v) : nt.firstChildTarget(g, p, v);
      }
      return f;
...
var Ps = function(t) {
  return { onCreated: function(n) {
    ...
    var e = n.$getConfig();
    e.order_branch && (e.order_branch == "marker" ? Ns.init(n.$gantt, n) : Ls.init(n.$gantt, n)), this.initEvents(n, t), n.$config.id == "grid" && this.extendDom(n);
...
const Rs = { createTaskDND: function() {
  var t;
  return { extend: function(n) {
    ...
    return t = function(i, a) {
      ...
      return { drag: null, dragMultiple: {}, _events: { before_start: {}, before_finish: {}, after_finish: {} }, _handlers: {}, init: function() {
        ...
        var d = i.$getConfig(), u = this._drag_task_coords(r, l);
        ...
        var d = this._drag_task_coords(r, l), u = i.$getConfig().rtl ? d.start - l.pos.x : l.pos.x - d.start, c = Math.max(0, u);
          ...
          if (this._handlers[o.mode]) {
            if (o.mode === l.drag_mode.move) {
              ...
              if (a.isSummaryTask(v) && a.config.drag_project) {
                var g = {};
                g[o.id] = J(o), _ = !0, h = H(g, this.dragMultiple);
              }
        ...
        if (r.button != 2 || r.button === void 0) {
          var l = i.$getConfig(), d = a.locate(r), u = null;
          if (a.isTaskExists(d) && (u = a.getTask(d)), !a.isReadonly(u) && !this.drag.mode) {
            this.clear_drag_state();
            var c = it(o = o || r.target || r.srcElement), h = this._get_drag_mode(c, o);
...
var Hs = function(t, n) {
  ...
  function g(f, p) {
    ...
    }(p), w = { right: (k = n.$root).offsetWidth, bottom: k.offsetHeight }, T = n.config.tooltip_offset_x || 10, S = n.config.tooltip_offset_y || 10, C = n.config.scroll_size || 18, E = n.$container.getBoundingClientRect().y + window.scrollY, A = { y: x.y + S, x: x.x + T, bottom: x.y + $.height + S + C, right: x.x + $.width + T + C };
  ...
  _.attachEvent("onBeforeDragStart", n.bind(function(f, p) {
    ...
    return !1;
  }, this)), _.attachEvent("onAfterDragStart", n.bind(function(f, p) {
    n.config.touch && n.refreshData(), y(_.config.marker);
  }, this)), _.attachEvent("onDragMove", n.bind(function(f, p) {
    var k = _.config, x = g(p, k.marker);
    (function(D, M) {
      D.style.left = M.x + "px", D.style.top = M.y + "px";
    })(k.marker, x);
    var $ = !!kt(p, h), w = i, T = e, S = a, C = n.locate(p), E = !0;
    if (Z(At(p), n.$root) || ($ = !1, C = null), $ && (E = !kt(p, "task_end_date"), $ = !!C), i = C, e = $, a = E, $) {
      const D = kt(p, h).querySelector(`.${c}`);
      if (D) {
        const M = Mn(D, t.$task_bg);
    ...
    var A = !(T == $ && w == C && S == E);
    return A && (w && n.refreshTask(w, !1), C && n.refreshTask(C, !1)), A && y(k.marker), b(this._dir_start.x, this._dir_start.y, this._dir_end.x, this._dir_end.y), !0;
  }, this)), _.attachEvent("onDragEnd", n.bind(function() {
    var f = o();
    if (f.link_source_id && f.link_target_id && f.link_source_id != f.link_target_id) {
      var p = n._get_link_type(f.link_from_start, f.link_to_start), k = { source: f.link_source_id, target: f.link_target_id, type: p };
      k.type && n.isLinkAllowed(k) && n.callEvent("onLinkCreated", [k]) && n.addLink(k);
    }
    m(), n.config.touch ? n.refreshData() : (f.link_source_id && n.refreshTask(f.link_source_id, !1), f.link_target_id && n.refreshTask(f.link_target_id, !1)), _._direction && (_._direction.parentNode && _._direction.parentNode.removeChild(_._direction), _._direction = null);
...
var Bs = function(t) {
  ...
  return { onCreated: function(e) {
    var i = e.$config;
    i.bind = U(i.bind) ? i.bind : "task", i.bindLinks = U(i.bindLinks) ? i.bindLinks : "link", e._linksDnD = Os(), e._tasksDnD = Rs.createTaskDND(), e._tasksDnD.extend(e), this._mouseDelegates = qe(t);
    ...
    this._mouseDelegates.delegate("click", "gantt_task_link", e.bind(function(a, s) {
      ...
      return r.parentNode && it(r.parentNode) && (l = it(r.parentNode).indexOf("_left") > -1 ? o.$target[0] : o.$source[0]), l && i.call(this, l, a), !1;
    }, e), this.$task);
  ...
  return { getVerticalScrollbar: function() {
    ...
    for (var e = n.getCellsByType("resizer"), i = 0; i < e.length; i++) {
      var a = e[i], s = !1, r = a.$parent.getPrevSibling(a.$id);
...
const js = { init: function(t) {
  ...
  var e = Ja(t);
  e.registerView("cell", wt), e.registerView("resizer", ns), e.registerView("scrollbar", is), e.registerView("layout", Qn, function(s) {
    (s.$config ? s.$config.id : null) === "main" && n(s, zs);
  }), e.registerView("viewcell", es), e.registerView("multiview", ts), e.registerView("timeline", Ye, function(s) {
    (s.$config ? s.$config.id : null) !== "timeline" && s.$config.bind != "task" || n(s, Bs);
  }), e.registerView("grid", Gt, function(s) {
    (s.$config ? s.$config.id : null) !== "grid" && s.$config.bind != "task" || n(s, Ps);
  }), e.registerView("resourceGrid", rs), e.registerView("GridRL", os), e.registerView("resourceTimeline", ti), e.registerView("resourceHistogram", ls);
  var i = function(s) {
    var r = Za(s);
  ...
  return t.ext.inlineEditors = a, t.ext._inlineEditors = a, a.init(t), { factory: e, mouseEvents: Xa.init(t), layersApi: i.init(), render: { gridLine: function() {
    return /* @__PURE__ */ function(s) {
      ...
      }, update: null, getRectangle: ie, isInViewPort: Ss, getVisibleRange: Q, onrender: function(r, o, l) {
    ...
    return /* @__PURE__ */ function(s) {
      ...
      }, getRectangle: Oe, getVisibleRange: Q, prepareData: $s };
    }(t);
  }, taskBar: function() {
    return function(s) {
      return { render: xe(s), update: null, isInViewPort: Mt, getVisibleRange: Q };
    }(t);
  }, timedProjectBar: function() {
    return function(s) {
      return { render: bs(s), update: null, isInViewPort: ys, getVisibleRange: Q };
    }(t);
    ...
    return function(s) {
      const r = xe(s), o = new Wn(s.$data.tasksStore), l = {};
      ...
      function u(_, g, y, m, b) {
        ...
        k && (x = cn(w)), b && (T = k ? Math.floor((x - w) / 2) + 2 : 2);
        ...
        if (k) {
          if (x > $) {
            ...
            const E = un($), A = (E - $) / 2, D = p.querySelector(".gantt_task_content");
      ...
      }, isInViewPort: xs, getVisibleRange: Q };
    }(t);
    ...
    return /* @__PURE__ */ function(s) {
      function r(o, l, d) {
        const u = document.createElement("div"), c = s.getTaskPosition(o, o.deadline, o.deadline), { height: h, marginTop: _ } = Un(s, l, c, 20, o, d);
        let g = h;
      ...
      }, isInViewPort: Is, getVisibleRange: Q };
    }(t);
    ...
    return /* @__PURE__ */ function(s) {
      function r(o, l, d, u, c) {
        ...
        if (g) {
          ...
          m = cn(S), b = Math.floor((m - S) / 4);
        }
        let v = ne(s, u, o, y.rowHeight).maxHeight, f = y.top + 1 + b, p = u.getBarHeight(o.id, o.type);
        ...
        switch (s.config.baselines.render_mode) {
          ...
          case "individualRow":
            ...
            break;
          default:
            w = 1, c ? (v = ne(s, u, o).maxHeight, b ? v >= m ? w = (v - p) / 2 - 1 - b : (p = un(v), f = y.top, w = Math.abs(p - v) / 2, b = 0) : (v > p && (w = (v - p) / 2 - 1), w -= b), o.bar_height || (w -= 1)) : (o.bar_height && y.rowHeight >= o.bar_height && (w = (y.rowHeight - o.bar_height) / 2 - 1), w += b, o.bar_height || (w += 2), g && (w += 1));
        }
        let T = y.top + v + 1 - b;
      ...
      }, isInViewPort: Ms, getVisibleRange: Q };
    }(t);
  }, link: function() {
    return ws(t);
...
function Fs(t) {
  var n = null, e = !1, i = null, a = { started: !1 }, s = {};
  function r(_) {
  ...
  function l(_) {
    if (i && (clearTimeout(i), i = null), _) {
      ...
      g && g < 10 && (g = 10), i = setTimeout(function() {
        n = setInterval(c, g || 50);
      }, t.config.autoscroll_delay || 10);
    }
  }
  function d(_) {
    _ ? (l(!0), a.started || (a.x = s.x, a.y = s.y, a.started = !0)) : (n && (clearInterval(n), n = null), l(!1), a.started = !1);
  }
  function u(_) {
    var g = o();
    if (!n && !i || g || d(!1), !t.config.autoscroll || !g) return !1;
    s = { x: _.clientX, y: _.clientY }, _.type == "touchmove" && (s.x = _.targetTouches[0].clientX, s.y = _.targetTouches[0].clientY), !n && g && d(!0);
  }
  function c() {
    if (!o()) return d(!1), !1;
    ...
    if (_) {
      ...
      var y = Y(_), m = s.x - y.x, b = s.y - y.y + window.scrollY, v = e ? 0 : h(m, y.width, a.x - y.x), f = h(b, y.height, a.y - y.y + window.scrollY), p = t.getScrollState(), k = p.y, x = p.inner_height, $ = p.height, w = p.x, T = p.inner_width, S = p.width;
...
const Ws = ["ctrlKey", "altKey", "shiftKey", "metaKey"], Vs = [[{ unit: "month", date: "%M", step: 1 }, { unit: "day", date: "%d", step: 1 }], [{ unit: "day", date: "%d %M", step: 1 }], [{ unit: "day", date: "%d %M", step: 1 }, { unit: "hour", date: "%H:00", step: 8 }], [{ unit: "day", date: "%d %M", step: 1 }, { unit: "hour", date: "%H:00", step: 1 }]];
class Us {
  constructor(n) {
    this.zoomIn = () => {
      ...
      a = typeof e.element == "function" ? e.element() : e.element, a && this._domEvents.attach(a, i, this.$gantt.bind(function(s) {
        if (this._useKey && (Ws.indexOf(this._useKey) < 0 || !s[this._useKey]))
          return !1;
...
function Be(t) {
  ...
  return F(e, n), e.prototype.render = function(i) {
    ...
    return s += ae.getHtmlSelect(i.options, [{ key: "style", value: "width:100%;" }, { key: "title", value: i.name }]), s += "</div>", s;
    ...
    (function(y, m) {
      ...
      var f = function(x) {
        ...
        C = E.checkbox, S = E.input, T = C.checked, w = S.value.trim(), E.filterApplied = !!w, t.getState().lightbox && (m = t.getLightboxValues()), $ = function(D, M, I, L) {
          ...
          return (P = t.copy(D)).options = wn(P.options, N), P;
        }(b, m, w, T);
        var A = m[b.map_to];
      ...
      f = Sn(f, 100), o(b).container.addEventListener("keyup", f), o(b).container.addEventListener("input", f, !0), o(b).container.addEventListener("change", f, !0), r(b).addEventListener("input", p), r(b).addEventListener("change", p), t.attachEvent("onResourcesSelectActivated", t.bind(p, r(b))), e.eventsInitialized[b.id] = !0;
  ...
  return t.resource_table = { scale_height: 35, row_height: 35, columns: [{ name: "resource", label: "Resource", align: "center", width: 80, editor: i, template: function(o) {
    ...
    if (function(y) {
      ...
      (function(b, v, f, p) {
        ...
        var k = function($) {
          ...
          S = C.input, T = S.value.trim(), e.resource_filter_value = T.toLowerCase(), C.filterApplied = !!T, t.getState().lightbox && (v = t.getLightboxValues()), w = function(A, D, M) {
            ...
            return I = function(N) {
              ...
              return P;
            }(D, A.options), L.resources = wn(L.resources, I), L;
          }(f, v, T);
          var E = v[f.map_to];
        ...
        k = Sn(k, 100), r(f).container.addEventListener("keyup", k), r(f).container.addEventListener("input", k, !0), r(f).container.addEventListener("change", k, !0), e.eventsInitialized[f.id] = !0;
  ...
  var n = function() {
    ...
    return F(k, p), k.prototype.render = function(T) {
      ...
      R[0].onclick = f.bind(function() {
        ...
        var G;
        return (G = (W = W || window.event).charCode || W.keyCode || W.which) == f.constants.KEY_CODES.DOWN ? (K(-1 * f.config.duration_step), !1) : G == f.constants.KEY_CODES.UP ? (K(1 * f.config.duration_step), !1) : void window.setTimeout(z, 1);
    ...
    function $(S, C, E, A) {
      ...
      var K, W, G, bt = z.getElementsByTagName("select"), Ht = z.getElementsByTagName("input"), Ot = Ht[1], Ke = [Ht[0], Ht[2]], si = z.getElementsByTagName("span")[0], ri = A._time_format_order;
      function xt() {
        var mt = w.call(f, z, A), X = T.call(f, z, A), oi = f.calculateEndDate({ start_date: mt, duration: X, task: E }), li = f.templates.task_end_date || f.templates.task_date;
        si.innerHTML = li(oi);
      }
      function qt(mt) {
        var X = Ot.value;
        X = x(A).parse(X), window.isNaN(X) && (X = 0), (X += mt) < 1 && (X = 1), Ot.value = x(A).format(X), xt();
      }
      z.querySelector(".baseline_delete_button").onclick = function(mt) {
        const X = z.parentNode;
        z.innerHTML = "", z.remove(), X.innerHTML === "" && (X.innerHTML = f.locale.labels.baselines_section_placeholder);
      }, Ke[0].onclick = f.bind(function() {
        qt(-1 * f.config.duration_step);
      }, f), Ke[1].onclick = f.bind(function() {
        qt(1 * f.config.duration_step);
      }, f), bt[0].onchange = xt, bt[1].onchange = xt, bt[2].onchange = xt, bt[3] && (bt[3].onchange = xt), Ot.onkeydown = f.bind(function(mt) {
        var X;
        return (X = (mt = mt || window.event).charCode || mt.keyCode || mt.which) == f.constants.KEY_CODES.DOWN ? (qt(-1 * f.config.duration_step), !1) : X == f.constants.KEY_CODES.UP ? (qt(1 * f.config.duration_step), !1) : void window.setTimeout(xt, 1);
      }, f), Ot.onchange = f.bind(xt, f), f._resolve_default_mapping(A), K = C.start_date || /* @__PURE__ */ new Date(), W = C.end_date || f.calculateEndDate({ start_date: K, duration: 1, task: E }), G = f.calculateDuration({ start_date: K, end_date: W, task: E }), G = x(A).format(G), f.form_blocks._fill_lightbox_select(bt, 0, K, ri, A), Ot.value = G, xt();
    }
    function w(S, C) {
...
<div class="gantt_cal_ltitle_controls">
  ...
  t._init_lightbox_events = function() {
    t.lightbox_events = {}, t.lightbox_events.gantt_save_btn = function() {
      ...
      if (p.getAttribute("data-dhx-button")) t.callEvent("onLightboxButton", [p.className, p, f]);
        ...
        var k, x, $ = it(p);
        if ($.indexOf("gantt_custom_button") != -1) if ($.indexOf("gantt_custom_button_") != -1) for (k = p.parentNode.getAttribute("data-index"), x = p; x && it(x).indexOf("gantt_cal_lsection") == -1; ) x = x.parentNode;
      ...
      var p = At(f), k = it(p);
      return k || (k = it(p = p.previousSibling)), p && k && k.indexOf("gantt_btn_set") === 0 && (k = it(p = p.firstChild)), !(!p || !k) && (t.defined(t.lightbox_events[p.className]) ? t.lightbox_events[p.className] : t.lightbox_events.default)(f, p);
    }), t.getLightbox().onkeydown = function(f) {
      var p = f || window.event, k = f.target || f.srcElement, x = it(k).indexOf("gantt_btn_set") > -1;
      switch ((f || p).keyCode) {
  ...
  }, template: new n(), textarea: new e(), select: new a(), time: new i(), duration: new o(), parent: new l(), radio: new r(), checkbox: new s(), resources: new d(), constraint: new u(), baselines: new h(), typeselect: new c(), resource_selector: new _() }, t._is_lightbox_timepicker = function() {
  ...
  if (!yt.isNode) {
    ...
    t.$ui = e.factory, t.$ui.layers = e.render, t.$mouseEvents = e.mouseEvents, t.$services.setService("mouseEvents", function() {
      ...
      return { getColumnIndex: function(c) {
        ...
        var _ = this, g = u.call(_, i, "getBarHeight", [c, h], function() {
      ...
      i.resetSkin || (i.resetSkin = function() {
        ...
        const o = this.skin !== r;
        this.skin = r, i._addThemeClass(), s(), i.$root && (Se(!o, i), this.render());
      };
      let a = null;
      function s() {
        const r = i.$root;
      ...
      i.ext || (i.ext = {});
      for (var a = [Fs, null, null], s = 0; s < a.length; s++) a[s] && a[s](i);
      i.ext.zoom = new Us(i);
    }(t), Gs(t), Ks(t), function(i) {
      i._extend_to_optional = function(a) {
        var s = a, r = { render: s.render, focus: s.focus, set_value: function(o, l, d, u) {
          ...
          return r.enable(o, u), s.set_value.call(i, o, l, d, u);
      ...
      for (var d in i._waiAria = { getAttributeString: function(u) {
        ...
        u.start_date && u.end_date && (c.setAttribute("aria-label", r(i.templates.tooltip_text(u.start_date, u.end_date, u))), u.$dataprocessor_class && c.setAttribute("aria-busy", !0));
        ...
        var h = i.config.links, _ = u.type == h.finish_to_start || u.type == h.start_to_start, g = u.type == h.start_to_start || u.type == h.start_to_finish, y = i.locale.labels.link + " " + i.templates.drag_link(u.source, g, u.target, _);
...
function Zs(t) {
  ...
  t.$data.tasksStore.hasChild = function(i) {
    ...
    if (t.config.branch_loading && t._load_url && function(l) {
      ...
      var a = t._load_url, s = (a = a.replace(/(\?|&)?parent_id=.+&?/, "")).indexOf("?") >= 0 ? "&" : "?", r = t.getScrollState().y || 0, o = { taskId: i, url: a + s + "parent_id=" + encodeURIComponent(i) };
...
const Qs = tt.gantt = function(t) {
  var n = Ga(t);
  return n.env.isNode || (Xs(n), function(e) {
